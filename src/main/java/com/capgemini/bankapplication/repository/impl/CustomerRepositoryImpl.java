...
import com.capgemini.bankapplication.repository.CustomerRespository;
...
public class CustomerRepositoryImpl implements CustomerRespository {
	...
	public Customer authenticate(Customer customer) throws DataAccessException {
		try {
			...
			e.initCause(new EmptyResultDataAccessException("Excepted 1 actual 0 ", 1));
			throw e;
		}
	}
	...
	public Customer updateProfile(Customer customer) {
		jdbcTemplate.update(
				"UPDATE customers SET customer_address = ?,customer_dob = ?,customer_email=?,customer_name=?   WHERE customer_id = ?",
